export const MODES = {
    veryEasy: {
        key: "veryEasy",
        name: "Very Easy",
        value: [15, 25],   // a random value will be selected between 15 and 25 
        mistakes: 5,
        hints: 6,
    },
    easy: {
...
export function removeCells(board, no) {
    for (let x = 0; x < no; x++) {
        const row = generateRandom(1, 9) - 1   // -1 is done to make the access as index as the index value starts from 0 so here it is minus 1 because to make the cell balanced according to the index  
